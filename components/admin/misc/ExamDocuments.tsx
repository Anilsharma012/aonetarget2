...
const ExamDocuments: React.FC<Props> = ({ showToast }) => {
  ...
  const handleSubmit = async () => {
    if (!formData.title || !formData.exam) { showToast('Fill required fields', 'error'); return; }
    try {
      const data = {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <button onClick={() => { setEditingItem(null); setFormData({ title: '', exam: '', fileUrl: '', status: 'active' }); setShowModal(true); }} className="bg-navy text-white px-6 py-3 rounded-xl font-black text-[11px] uppercase shadow-sm hover:shadow-md hover:scale-105 transition-all flex items-center gap-2"><span className="material-icons-outlined text-base">add</span> Add</button>
      </div>
