        ...
        <div className="glass rounded-2xl p-1.5 mb-6 shadow-card">
          <div className="flex gap-1">
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl font-semibold text-xs flex items-center justify-center gap-2 transition-all duration-200 active:scale-[0.97] ${
                activeTab === 'live'
        ...
        {loading ? (
          <div className="space-y-4">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="skeleton h-28 w-full" style={{ animationDelay: `${i * 0.15}s` }}></div>
            ))}
          </div>
        ) : activeTab === 'live' ? (
          liveClasses.length > 0 ? (
