      ...
      <div className="p-4">
        <div className="flex gap-2 overflow-x-auto pb-3 mb-5 hide-scrollbar">
          <button
            ...
            className={`px-4 py-2 rounded-full font-semibold text-xs whitespace-nowrap transition-all duration-200 active:scale-[0.97] flex items-center gap-1.5 ${
              selectedCourse === 'all'
                ? 'btn-primary'
          ...
          >
            ...
            <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
              selectedCourse === 'all' ? 'bg-white/20 text-white' : 'bg-surface-200 text-gray-500'
            }`}>{tests.length}</span>
          </button>
          {coursesWithTests.map(group => (
            ...
            >
              {group.courseName}
              <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                selectedCourse === group.courseId ? 'bg-white/20 text-white' : 'bg-surface-200 text-gray-500'
                                ...
                                return (
                                  <div key={test.id} className="flex items-center justify-between py-3.5 border-b border-surface-100 last:border-0 animate-fade-in-up" style={{ animationDelay: `${tIdx * 0.05}s` }}>
                                    ...
                                    <button
                                      ...
                                      className={`px-4 py-2 rounded-xl text-[11px] font-bold transition-all duration-200 active:scale-[0.97] ${
                                        status === 'upcoming'
                      ...
                      return (
                        <div key={test.id} className="card-premium p-4 animate-fade-in-up" style={{ animationDelay: `${tIdx * 0.06}s` }}>
                          ...
                          <button
                            onClick={() => status !== 'upcoming' && navigate(`/test/${test.id}`)}
                            disabled={status === 'upcoming'}
                            className={`w-full mt-4 py-3 rounded-2xl text-xs font-bold flex items-center justify-center gap-2 transition-all duration-200 active:scale-[0.97] ${
                              status === 'upcoming'
                                ? 'bg-surface-200 text-gray-400 cursor-not-allowed'
                                : status === 'completed'
